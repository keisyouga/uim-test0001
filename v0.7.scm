...
(define-key test0001-begin-conv-key? 'generic-begin-conv-key?)
...
(define test0001-commit
  (lambda (context)
    (if (test0001-context-candidate-window context)
        (let ((cand (nth (test0001-context-cand-nth context) test0001-cands)))
          (im-commit context cand)))
...
(define test0001-proc-on-state
  (lambda (context key key-state)
    (cond
     ...
     ;; conv
     ((test0001-begin-conv-key? key key-state)
      (let ((seq (test0001-context-seq context)))
        (if (null? seq)
            (im-commit-raw context)
            ;; seq => unicode
            (let ((num (string->number
                        (apply string-append (reverse seq)) 16)))
              (if (and num (< num #xfffff)) ; workaround
                  (begin
                    (im-commit context (ucs->utf8-string num))
                    (test0001-clear context)
                    ))))))
     ;; commit
     ((test0001-commit-key? key key-state)
      (if (test0001-context-candidate-window context)
          ...
          (if (pair? (test0001-context-seq context))
              (begin
...
(define test0001-set-candidate-index-handler
  (lambda (context idx)
    (uim-notify-info "set-candidate-handler")
    #f))
